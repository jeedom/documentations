...
Il faut aller sur le panneau de configuración puis "Services de
...
(j'ai pas trouvé l'info sur le client web) et aller dans configuración →
stockage :
...
ensuite retourner avec le client lourd dans configuración → stockage :
...
vous faudra sûrement vous connecter avec votre compte VMware). En la
...
Ensuite sur le NAS dans le panneau de configuración puis portail
...
> añadir. Ensuite n'oubliez pas de cliquer sur configurer pour
