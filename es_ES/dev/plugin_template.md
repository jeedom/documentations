...
Puede encontrar una base de complementos vacía [aquí](https://github.com/jeedom/plugin-template)
...
- ``core`` : Carpeta que contiene todos los archivos de trabajo internos.
  ...
  - ``ajax`` : Carpeta que contiene archivos de destino de llamada AJAX.
  - ``i18n`` : Carpeta que contiene archivos .plugin de traducción json.
- ``desktop`` : Carpeta que contiene la vista "de escritorio" del complemento (a diferencia de la vista "móvil"").
  ...
  - ``template`` : Carpeta que contiene plantillas html para mosaicos específicos para equipos de complementos, en el tablero y subcarpetas móviles.
- ``plugin_info`` : Contiene los archivos que permiten a Jeedom calificar el complemento, instalarlo y configurarlo.
  ...
  - ``configuration.php`` : Archivo que contiene los parámetros a configurar para el complemento independientemente de su equipo (ejemplo para el módulo Zwave, la IP Raspberry Pi con la tarjeta Razberry)
- ``docs`` : Debe contener el documento del complemento en formato de descuento, raíz y archivo index.md. Todas las imágenes están en documentos/imágenes. El documento en sí está en una carpeta según el idioma (por ejemplo, en francés : ``docs/fr\_FR``)
- ``ressources`` : Carpeta para posibles demonios y dependencias.
- ``data`` : Carpeta utilizada para los archivos generados por el complemento específico del Jeedom del usuario.
...
- si no es manejado por un archivo de inclusión, el nombre del archivo debe ser ``nom\_class.class.php``
- los archivos que solo sirven como punto de entrada para incluir varios archivos deben terminar con ``.inc.php``
...
- el nombre del archivo JS (si lo hay) de la primera página de vista del complemento debe ser la ID del complemento
...
La primera parte comentada contiene la licencia (es mejor). El que se usa aquí indica que el archivo pertenece a Jeedom y que es de código abierto Luego viene la inclusión del núcleo de Jeedom (esto permite el acceso a las funciones internas) Luego vienen las 2 funciones :
...
Esta carpeta contiene la vista actual. Dentro debemos encontrar la página de configuración del plugin (la que aparecerá cuando el usuario haga plugin ⇒ categoría ⇒ tu plugin). Es recomendable nombrarlo con el id de tu plugin. También puede contener el panel (página que el usuario encontrará en home → nombre de su complemento).

Todos los archivos en esta carpeta deben terminar con ``.php`` y debe empezar con :

````php
<?php
si (!esConectar('administrador')) {
    throw new Exception('{{401 - Acceso no autorizado}}');
 }
...
Para poner en la parte inferior de su página y útil solo en la página de configuración de su complemento. Este script reduce el javascript obligatorio a una sola función (consulte la sección sobre archivos JS).

En su página de configuración, se ha implementado una sintaxis HTML para facilitarle la vida. Entonces, para la mayoría de los complementos, solo tendrá que hacer HTML para almacenar su información en la base de datos y, por lo tanto, usarla nuevamente en el lado de su clase.
...
Para ver la lista de propiedades del equipo y los comandos, está aquí (para ver las propiedades que son JSON, solo mire el getter o el setter, si toma 2 parámetros, entonces es JSON)

Último punto importante en la página de configuración: puede contener tantos dispositivos y comandos como sea necesario. Sin embargo, hay algunas reglas a seguir :

Todos los elementos con clase eqLogicAttr deben estar en un elemento con clase css eqLogic Lo mismo ocurre con los elementos de clase css cmdAttr que deben estar en un elemento de clase cmd. Todos los comandos de un dispositivo deben estar en el elemento con la clase eqLogic correspondiente

## JS

Todos los archivos JS deben estar en la carpeta JS (fácil !!!). Es recomendable nombrarlo con el mismo ID que tu plugin (en la parte de configuración, para el panel haces como quieras). Este archivo JS (el de la configuración del complemento) debe contener al menos un método addCmdToTable que toma como parámetro el objeto de comando para agregar. Aquí hay un ejemplo simple :
...
- ``jeedom.cmd.availableType()`` insertará una selección con la lista de tipos conocidos (acción e información por ahora)
- ``<span class="subType" subType="' + init(\_cmd.subType) + '"><\span>`` : dónde debe colocarse la selección de subtipo
...
- ``printEqLogic`` que toma en parámetros todo el objeto del equipo (útil en caso de procesamiento de datos antes de restaurarlos). Se llama cuando se muestran los datos del equipo
- ``saveEqLogic`` que toma como parámetro el objeto del equipo que se guardará en la base de datos (útil si tiene que hacer algún procesamiento antes de guardar) Por último, para los archivos JS, aquí le mostramos cómo incluirlos de manera limpia en su página php :
...
Le premier argument donne le dossier dans lequel le trouver (attention c'est le dossier père du dossier CSS), le deuxième le nom de votre fichier css, le troisième indique à Jeedom que c'est un fichier CSS et le dernier dans quel plugin se encuentra.

## MODAL

La carpeta modal le permite almacenar sus archivos php destinados a mostrar modales. Aquí se explica cómo llamarlos desde su página principal (este código se coloca en un archivo javascript) :
...
Cuando hacemos clic en el botón del asistente, obtenemos la entrada en la que escribir y luego llamamos al asistente. Una vez finalizada la configuración en el asistente, el resultado se recupera y luego se escribe en la entrada previamente seleccionada
...
Nota : a lo largo de esta parte, se hará referencia a la identificación de su complemento : plugin\_id

## PHP

Contiene los archivos PHP adicionales, solía poner por ejemplo un archivo de inclusión si, por supuesto, tiene varios archivos de clase o 3rparties para incluir

## Template

Que puede contener 2 subcarpetas, tablero y móvil, es una carpeta que Jeedom escanea automáticamente en busca de widgets, por lo que si usa widgets específicos, aquí es donde debe colocar su archivo HTML

## i18n

Es aquí que su traducción debe estar en forma de archivo json (lo mejor y para mirar por ejemplo en el complemento [onda z](https://github.com/jeedom/plugin-openzwave) para ver la forma del archivo)
...
probar {
    ...
    si (!esConectar('administrador')) {
        throw new Exception(__('401 - Acceso no autorizado', __FILE__));
    }

    if (init('acción') == 'tu acción') {
        ajax::exito($resultado);
    }

    throw new Exception(__('Ningún método coincidente : ', __EXPEDIENTE__) . init('acción'));
    /*     * *********Excepción de lucha libre*************** */
} catch (Excepción $e) {
...
Para la definición de las clases de jeedom, los invito a consultar este [sitio](https://doc.jeedom.com/dev/phpdoc/4.0/)
...
función pública ejecutar ($_opciones = matriz()) {
      si (!isset($_opciones['título']) && !isset($_opciones['mensaje'])) {
          throw new Exception(__("Título o mensaje no pueden estar ambos vacíos", __FILE__));
      }
      $eqLogic = $this->getEqLogic();
      $message = '';
      if (isset($_opciones['título'])) {
...
Para convertir la versión solicitada a tablero o móvil (mview se convierte en móvil, por ejemplo, esto permite, por ejemplo, en las vistas agregar el nombre de los objetos)
...
Aquí reemplazo de las etiquetas rellenadas previamente en \$replace HTML para contener los valores
...
Aquí te permite poner el valor en la etiqueta, solo si la orden ha sido recuperada con éxito
...
````php
cache::set('widget del tiempo' . $_version . $this->getId(), $html, 0);
````
...
También debe decirle a Jeedom qué permite su widget para la personalización. Es un poco complejo (y aún así) pero normalmente flexible y fácil de configurar.

Funciona de la misma manera en su equipo o comando, es un atributo estático de la clase \$\_widgetPossibility que debe ser una matriz multidimensional, pero ahí es donde se complica si una dimensión de la matriz es verdadera o falsa. Luego considera que todos los hijos posibles están en este valor (daré un ejemplo).

En primer lugar los casos en los que hay que utilizarlo: si en su clase que hereda de eqLogic o cmd tiene una función toHtml, de lo contrario no vale la pena seguir leyendo.
...
- ``preSave`` ⇒ Método llamado antes de guardar (por lo tanto, crear y actualizar) su objeto
...
Ejemplo, aún con el complemento meteorológico de crear pedidos o actualizarlos después de guardar (el ejemplo está simplificado) :

````php
publicación de la función públicaActualización() {
      $weatherCmd = $this->getCmd(null, 'temperature');
      si (!is_object($weatherCmd)) {
...
- Para subir la batería (en %) de un equipo, hazlo sobre él (Jeedom se encargará del resto y avisará al usuario si es necesario)) :
