...
Ver [](https://doc.jeedom.com/es_ES/dev/structure_info_json)
...
Ce n'est pas un dossier mais dans les dernières versions de Jeedom celui-ci offre au développeur toute une api javascript (cela évite d'écrire des appels Ájax dans tous les sens). J'essayerai de faire un article pour expliquer les différentes fonctionnalités mais vous pouvez déjà trouver le code .
...
Récupération d'un template de commande,  le template de commande : plugins/weather/core/template/\$\_version/forecast. (\$\_version valant mobile ou dashboard)
