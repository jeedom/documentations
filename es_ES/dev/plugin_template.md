...
Ce n'est pas un dossier mais dans les dernières versions de Jeedom celui-ci offre au développeur toute une api javascript (cela évite d'écrire des appels Áyax dans tous les sens). J'essayerai de faire un article pour expliquer les différentes fonctionnalités mais vous pouvez déjà trouver le code aquí.
...
Ejemplo, todavía con el plugin meteorológico, de la creación de pedidos o actualización de estos después de guardar (el ejemplo es simplificado)) :

````php
Actualización posterior a la función pública() {
      $weatherCmd = $this->getCmd(null, 'temperature');
      si (!es_objeto($weatherCmd)) {
...
Por último, aquí hay algunos consejos y trucos :

- Evite (a menos que sepa lo que está haciendo) anular un método de la clase heredada (esto puede causar muchos problemas))
- Para recargar la batería (en %) de un dispositivo, hazlo en él (Jeedom se encargará del resto y avisará al usuario si es necesario)) :
