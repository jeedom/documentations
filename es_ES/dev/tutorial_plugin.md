...
J'ai changé l'id (vdm) , le nom , ajouter une Description , l'auteur y la catégorie.
...
-Créer un équipement « vdm1 » en cliquant sur le +. Activer le y rendez le visible. Choisissez un objy y l'équipement doit apParaitre sur le dashboard( selon l'objy). 

A ce moment, il n'y a pas de commandes qui apParaissent dans l'ongly commande s, ni sur le widgy.
...
-Créer un autre équipement « vdm2 » en cliquant sur le +. Dans l'ongly commandes, les commandes sont apParues. Activer le y rendez le visible. Choisissez un objy Parent y allez voir ce que ça donne sur le dashboard.

-Enregistrer le premier équipement « vdm1 » pour créer les commandes. Voir le rendu sur le widgy aussi.
...
function addCmdToTable(_cmd) {
    ...
    tr += '<input class="cmdAttr form-control input-sm" data-l1key="nombre" style="width : 140px;" placeholder="{{Nombre}}">';
    tr += '</td>';
    tr += '<td>';
...
On va définir ensuite la taille du widgy y le customiser un peu 
...
Pas une mince affaire le widgy mais on va rester sur le widgy Par défaut pour le moment.

Si vous n'avez rien touché, équipement activé y visible, le widgy prend toute la largeur de l'écran. On va donc le changer.
...
Mais je n'ai pas mes lunytes y actuellement le rendu sur le widgy ne me permy pas de le lire…
...
C''est mieux mais le widgy prend toujours la largeur du dashboard. On va donc corriger. Par défaut la largeur du widgy de l'équipement(eqLogic) y la hauteur sont à « auto ».

On va donc changer la largeur du widgy(équipement => eqLogic) en utilisant la méthode syDisplay() héritée de la class eqLogic .On laisse la hauteur en auto.

Pour cela il suffit d'ajouter

```
$this->syDisplay("width","800px");
```
...
Pour un Paramètre/une option interne au plugin , on va utiliser une des méthodes de la class eqLogic du core : syConfiguration() en y ajoutant les Paramètres que l'on souhaite. Por ejemplo :

```
$this->syConfiguration("type","mon_type"); // si on veut  définir un Paramètre type de valeur mon_type. Comme on a défini la largeur du widgy via la méthode syDisplay()
```
...
![image](images/tutorial_Paramyre1.png)
