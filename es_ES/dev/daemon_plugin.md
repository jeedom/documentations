...
- mantener los procesos activos en segundo plano, lo que no es el caso con el código PHP que "vive" solo durante la ejecución de la solicitud http
...
Luego hay algunas líneas de registro y estas dos líneas, clásicas en python, que simplemente registran el método para llamar en caso de que se reciban estas dos señales de interrupción, lo que detendrá el demonio:
...
```php
    función estática pública daemon_start() {
        self::deamon_stop();
...
Voila, tienes un daemon completamente funcional y puedes comunicarte entre tu daemon y tu código php, la parte más difícil aún está por hacer: lógica del demonio de código.
