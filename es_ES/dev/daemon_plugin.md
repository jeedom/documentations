...
```php
    función estática pública deamon_info() {
        ...
        $return['state'] = 'nok';
        $pid_file = jeedom::obtenerTmpFolder(__CLASS__) . '/deamon.pid';
...
```php
    función estática pública deamon_start() {
        ...
        $cmd .= ' --pidentificación ' . jeedom::obtenerTmpFolder(__CLASS__) . '/deamon.pid'; // et on précise le chemin vers le pidentificación file (ne pas modifier)
...
```php
    función estática pública deamon_stop() {
        $pid_file = jeedom::obtenerTmpFolder(__CLASS__) . '/deamon.pid'; // ne pas modifier
        si (archivo_existe ($pid_archivo)) {
...
```php
    función estática pública dependency_install() {
        ...
        return array('script' => dirname(__FILE__) . '/../../resources/install_#stype#.sh ' . jeedom::obtenerTmpFolder(__CLASS__) . '/dependencia', 'registro' => registro::getPathToLog(__CLASS__. '_update'));
    }
```
...
El primer argumento: `jeedom::obtenerTmpFolder(__CLASS__) . '/dependence'` es el archivo utilizado para monitorear el progreso de la instalación (el porcentaje que aparece en la pantalla durante la instalación).
...
```php
    función estática pública dependency_info() {
        ...
        $return['progress_file'] = jeedom::obtenerTmpFolder(__CLASS__) . '/dependance';
        si (file_exists(jeedom::obtenerTmpFolder(__CLASS__) . '/dependance')) {
