...
## Introduction
 ...
 [](/es_ES/dev/tutorial_plugin)  [documentación](/es_ES/dev/plugin_template) ).
.
...
```php
    () {
        ...
        log::
        $result = exec($cmd . ' >> ' . log::getPathToLog('template_daemon') . ' 2>&1 &'); // 'template_daemon' est le nom du  pour votre démon, vous devez nommer votre  en commençant par le pluginidentificación pour que le fichier apparaisse dans la page de config
        $i = 0;
        ) {
            $deamon_info = self::deamon_info();
            ') {
