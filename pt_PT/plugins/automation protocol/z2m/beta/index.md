...
- Ação : `temperature::25` para enviar `{"temperatura":25}` no tópico `set` do equipamento ou `arm_mode/mode::arming_stay` para enviar `{"arm_mode":"arming_stay"}` no tópico `set/arm_mode` do equipamento
