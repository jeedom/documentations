...
``category`` *               | Catégorie de classement du plugin sur le Market jeedom. **Respecter impérativement la [nomenclature du tableau ci-dessous](#nomenclature-categories)** |
...
``hasDependency``            | «true» si [le plugin doit installer des dépendances](daemon_plugin.md#les-dépendances), sinon «false» ou absent.                                              |
``hasOwnDeamon``             | «true» si [le plugin doit exécuter des deamons](daemon_plugin.md#les-démons--dépendances), sinon «false» ou absent.                                                   |
``maxDependancyInstallTime`` | Temps maximum imparti pour l’installation des dépendances, exprimé en minutes.                                            |
``specialAttributes`` | Permet aux plugin de demander des [parametre suplémentaire](#specialattributes) sur [des objets](#attributs-objet) ou [des utilisateurs](#attributs-user) (bien regarder l'exemple du plugin template) (Voir explicationsci-dessous)                                            |
