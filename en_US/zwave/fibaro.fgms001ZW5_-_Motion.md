...
Once included, you must apply the zwave + configuration via the drop-down list, you should get this :

![Plugin Zwave](images/fibaro.fgms001zw5/information.jpg)

### Commands

You have to click once on the magnifying glass to retrieve the module commands. Once the module is recognized, the commands associated with the module will be available.
...
-   Seismic : it is the command which makes it possible to raise the seismic intensity
...
> **Tip**
>
> This module has five association groups, add the controller on 1, 4 and 5 and remove the 3.

The names of the groups in the Z-Wave + version are as follows:

-   1 : Lifeline, module status feedback. The primary controller should be added to this group.
-   2 : Motion, motion sensor.
-   3 : Tamper, sabotage alert.
-   4 : Motion BC, motion sensor. The purpose of this group is to ensure backward compatibility with controllers that do not support the Z-Wave + protocol.
-   5 : Tamper BC, sabotage alert. The purpose of this group is to ensure backward compatibility with controllers that do not support the Z-Wave + protocol.
...
This module is very finicky. It is advisable to make the inclusion as close as possible to your box and to repeat it several times.
