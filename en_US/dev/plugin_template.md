...
- ``desktop`` : Folder containing the "desktop" view of the plugin (as opposed to the "mobile" view").
  ...
  - ``php`` : Folder containing all php type files for plugin interface.
  - ``css`` : If needed, all the css files of the plugin, possibly including *fonts*.
  - ``modal`` : Folder containing the plugin modals code.
  - ``img`` : Folder for images (png, jpg etc) needed by the plugin.
  - ``template`` : Folder containing html templates for tiles specific to plugin equipment, in the dashboard and mobile sub-folders.
- ``plugin_info`` : Contains the files allowing Jeedom to qualify the plugin, to install and configure it.
  ...
  - ``install.php`` : File containing (if needed) the methods for installing and uninstalling the plugin.
  - ``configuration.php`` : File containing the parameters to be configured for the plugin independent of its equipment (example for the Zwave module, the Raspberry Pi IP with the Razberry card)
- ``docs`` : Must contain plugin doc in markdown format, root and index.md file. All images are in docs/images. The doc itself is in a folder depending on the language (eg in French : ``docs/fr\_FR``)
- ``ressources`` : Folder for possible daemon and dependencies.
...
- if not handled by an include file, the file name should be ``nom\_class.class.php``
- files serving only as an entry point to include multiple files must end with ``.inc.php``
...
- the name of the JS file (if there is one) of the first view page of the plugin should be the plugin ID
...
The first commented part contains the license (it's better). The one used here indicates that the file belongs to Jeedom and that it is open source Then comes the inclusion of Jeedom's core (this allows access to internal functions) Then come the 2 functions :
...
This folder contains the actual view. Inside we must find the configuration page of the plugin (the one that will appear when the user makes plugin ⇒ category ⇒ your plugin). It is advisable to name it with the id of your plugin. It can also contain the panel (page that the user will find in home → name of your plugin).
...
To put at the very bottom of your page and useful only on the configuration page of your plugin. This script reduces the mandatory javascript to a single function (see section on JS files).

In your configuration page, an HTML syntax has been implemented to make your life easier. So for most plugins you will only have to do HTML to store your information in the database and therefore use it again on the side of your class.
...
For the list of properties of equipment and commands it's here (to see the properties that are JSON just look at the getter or the setter, if it takes 2 parameters then it's JSON)

Last important point on the configuration page: it can contain as many devices and commands as necessary. However, there are a few rules to follow :

All elements with class eqLogicAttr must be in an element with css class eqLogic Ditto for elements of css class cmdAttr which must be in an element of class cmd. All the commands of a device must be in the element with the corresponding eqLogic class

## JS

All JS files should be in the JS folder (easy !!!). It is advisable to name it with the same ID as your plugin (in the configuration part, for the panel you do as you want). This JS file (that of the configuration of the plugin) must at least contain an addCmdToTable method which takes as a parameter the command object to add. Here is a simple example :
...
- ``printEqLogic`` which takes in parameter all the object of the equipment (useful in the event of processing of data before restoring them). It is called when displaying equipment data
- ``saveEqLogic`` which takes as a parameter the equipment object that will be saved in the database (useful if you have to do some processing before saving) Last thing, for JS files, here is how to include them in a clean way on your php page :
...
The first argument gives the folder in which to find it (note this is the parent folder of the CSS folder), the second the name of your css file, the third tells Jeedom that it is a CSS file and the last in which plugin it turns out that.

## MODAL

The modal folder allows you to store your php files intended to display modals. Here is how to call them from your main page (this code is put in a javascript file) :
...
When we click on the assistant button, we get the input in which to write and then we call the assistant. Once the configuration is finished in the wizard, the result is retrieved then written to the previously selected input
...
Contains the additional PHP files, I used to put for example an inclusion file if, of course, you have several class files or 3rparties to include
...
It is here that your translation must be in the form of a json file (the best and to look for example at the plugin [zwave](https://github.com/jeedom/plugin-openzwave) to see the shape of the file)

## ajax

This folder is for all your ajax files, here is a skeleton ajax file :
...
Here replacement of the tags previously filled in \$replace HTML to contain the values
...
Here it allows you to put the value in the tag, only if the order has been successfully retrieved
...
Save widget to cache: so that during the next request we provide it more quickly, we can notice the 0 here which indicates an infinite lifespan, otherwise the duration is in seconds (we will see in the next part how the weather plugin updates its widget).
...
You must also tell Jeedom what your widget allows for customization. It's a little complex (and still) but normally flexible and simple to set up.

It works the same way on your equipment or command, it's a static attribute of the class \$\_widgetPossibility which must be a multidimensional array, but that's where it gets complicated if a dimension of the array is a true or false. It then considers that all the possible children are at this value (I will give an example).

First of all the cases where you have to use it: if in your class inheriting from eqLogic or cmd has a toHtml function otherwise it is not worth reading further.
...
- ``preSave`` ⇒ Method called before saving (therefore creating and updating) your object
...
Example, still with the weather plugin of creating orders or updating them after saving (the example is simplified) :
...
- Avoid (unless you know what you're doing) overriding a method of the inherited class (this can cause a lot of trouble)
- To raise the battery (in %) of a device, do it on it (Jeedom will take care of the rest and warn the user if necessary) :
