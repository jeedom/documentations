...
- ``core`` : Folder containing all internal operating files.
  ...
  - ``ajax`` : Folder containing the AJA call target filesX.
...
- ``desktop`` : Folder containing the "desktop" view of the plugin (as opposed to the "mobile" view").
  - ``js`` : Folder containing all the javascript type files for the plugin interface.
  - ``php`` : Folder containing all the php type files for the plugin interface.
  - ``css`` : If necessary, all the plugin's CSS files, including possibly *fonts*.
  - ``modal`` : Folder containing the plugin's modals code.
  - ``img`` : Folder for images (png, jpg etc) needed by the plugin.
- ``plugin_info`` : Contains the files allowing Jeedom to qualify the plugin, to carry out its installation and configuration.
...
- files that serve only as an entry point to include multiple files must end with ``.inc.php``
...
- the name of the JS file (if any) of the plugin's first view page must be the plugin ID
...
This folder contains the view itself. Inside we necessarily find the plugin configuration page (the one that will appear when the user does plugin ⇒ category ⇒ your plugin). It is recommended to name this one with the ID of your plugin. It can also contain the panel (page that the user will find in home → name of your plugin).
...
In your configuration page, an HTML syntax has been implemented to make your life easier. So for most plugins you will only have to do HTML to store your information in the database and therefore use it again on your class side.
