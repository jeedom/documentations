...
```
{{aux dernières de l'équipement template}}
```

Par

```
{{aux dernières de l'équipement }} 
```
...
```
                    <input type="text" class="eqLogicAttr form-control" data-l1key="name" placeholder="{{aux dernières de l'équipement}}"/>// retourne le nom de l'eqLogic(équipement) . Qu'on va pouvoir récupérer via $this->getName() ;
```
