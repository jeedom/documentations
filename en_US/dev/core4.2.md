...
Just add this button to the right of the search field on your page `myplugin/desktop/php/myplugin.php` :
...
When opening this page, the Core checks, for each plugin, if it has a `pluginGenericTypes()` method. If this is the case, this method is then called, waiting for the Generic Types of the plugin in order to integrate them. These must respect the definition of Generic Types of the Core, especially if categories already exist (Grip, Light, etc.).).
