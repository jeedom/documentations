...
In v4.2, on the page **Analysis → Equipment**, Orphaned Commands tab, the eqLogic `deadCmdGeneric()` function now returns a link to the affected scenario or equipment.
...
<details>
  ...
  So you can integrate the same type of return into your plugins, `deadCmd()` function.
...
So this needs to be tested and integrated for each plugin. Several possible cases :
...
If you would like an item from the *display card* appears on the right, add the CSS class `displayTableRight`. If you need to place multiple items in it, place them all in one ` <span class="displayTableRight">...</span> `
