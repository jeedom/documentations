...
A notable difference in mobile is the absence of PHP pages. Code generation therefore relies on JS classes, which can still call Core functions with AJAX calls.
...
- `DB.class.php` : This section groups together all the functions for accessing the Jeedom database. All SQL access required by other classes is handled by this one.
