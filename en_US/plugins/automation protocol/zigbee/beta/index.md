...
-	**)** : Tested, validated and recommended by the Jeedom team.
-	**XBee** : Not tested by the Jeedom team.
-	**** : Not tested by the team.
-	**)** : Not tested by the team.
...
-  **backup and restore** a controller,
-  **Firmware update** a controller,
-  **update of modules** in OTA,
...
You can find [](https://compatibility.jeedom.com/index.php?v=d&p=home&plugin=zigbee) the list of modules compatible with the plugin
...
>****
>
>Due to a bug in some firmware *(Ikea, Sonoff, etc)*, it is sometimes necessary to choose the type of module directly from the list **** then save so that the orders are created correctly.
...
- **** : unique identifier of the equipment. Even during a re-inclusion or if you change the type of Zigbee controller.
...
The part **** allows manual selection of manufacturer and equipment. There is also the visual of the equipment as well as two buttons allowing the **regeneration of orders** or access to options **Module configuration**.

In the tab ****, we find, as usual, the commands allowing to interact with the module.
...
**** : ````attributes::1::in::513::18::#slider#*100```` will write attribute to endpoint `1`, incoming cluster (````in````) `513`, attribute` 18` with the value of the ````slider*100````.
...
**** : ````1::on_off::on````, execute the command ````on```` on the endpoint `1` of the cluster ````on_off```` without parameters.        
**** : ````1::level::move_to_level::#slider#::0````, execute the command ````move_to_level```` on the endpoint `1` of the cluster ````level```` With parameters ````#slider#````  ````0````.
...
-  ****  ****, the firmwares are directly made available online where the plugin will retrieve them.
- For others (see [](https://github.com/Koenkk/zigbee-OTA/tree/master/images)), the manufacturer informally provides an update in some cases.
...
Updates are carried out automatically in the event of availability or if the module requests it. It is possible to force the update of a module from the tab **** from the module configuration window on the device page.
...
**** *(or Lightlink)* is a particular function of Zigbee which allows the controller to send management orders to a module on condition of being very close to it *(less than 50 centimeters)*. This is useful, for example, to reset bulbs that do not have a physical button.
...
You will find the binding management elements, if it is supported by your module, in the tab **** from the module configuration window.
...
>**I have errors ````can not send to device````  ````send error````  ````Message send failure````**
>
...
> -	the module is in ZHA, so it's a concern of inclusion. In the tab **** of the module configuration, there is a button **Reset module** allowing to force post-inclusion actions. Care must be taken to keep the module awake if it is on battery.
...
>  : this manipulation should only be done with Elelabs, Jeedom Atlas or Jeedom Smart type keys. On any other key you must contact the manufacturer to give you the procedure.
