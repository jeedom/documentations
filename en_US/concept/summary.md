...
- **Hide number if zero** : Allows you to hide the summary value, only if it is 0. We can thus wish to have the open shutter icon with their number, and the closed shutter icon without the number when all the shutters are closed.
