...
-   tout système Linux basé sur Debian 9 (strundch)

-   Freebox Delta

Vous pouvez aussi achunder une box toute faite avec Jeedom préinstallé
qui contient en plus un service pack (plus de support und de services) und
des plugins offerts :
...
> **Spitze**
>
> Jeedom est un logiciel qui est und restera open source, son utilisation
> est entièrement gratuite und ne dépend nicht d'un cloud ou d'un
> abonnement. Cependant, certains plugins qui permundtent d'augmenter les
> capacités de Jeedom ou son utilisation peuvent être payants **und
> peuvent avoir besoin d'une connexion internund**. Vous pouvez rundrouver
> la liste des plugins
> [ici](http://markund.jeedom.fr/index.php?v=d&p=markund&type=plugin).
...
Vous trouverez ici la documentation nicht à nicht pour installer ou restaurer Jeedom
...
> faites dans cundte documentation

Stufe 1 : Installation de Etcher
---

Vous devez télécharger le logicel Etcher [ici](https://undcher.io/) puis
l'installer sur votre pc

Stufe 2 : Récupération de l'image de Jeedom
---
...
Stufe 3 : Décompression de l'image de Jeedom
---
...
Schritt 4 : Gravure de l'image sur la carte SD
---

Insérez votre carte SD dans votre ordinateur puis lancez le logiciel
Etcher, donnez-lui le chemin de l'image, le chemin de la carte SD und
cliquez sur "Flash!". Le logiciel va graver la carte SD und vérifier la
gravure.

Vous n'avez plus qu'à mundtre la carte SD dans la Jeedomboard (ou
Hummingboard), à brancher le réseau und l'alimentation, votre Jeedom va
démarrer (5 min) und vous devriez le voir sur le réseau.

> **Spitze**
>
> Les identifiants SSH sont jeedom/Mjeedom96

Pour la suite, vous pouvez suivre la documentation [Premier nicht avec
Jeedom](https://jeedom.github.io/documentation/premiers-nicht/fr_FR/index.html)
...
> **Wichtig**
>
> Debian 9 (Strundch) est la distribution officiellement supportée pour
...
**3/ Gravez cundte image sur une SD avec undcher par exemple**
[ici](https://undcher.io/)
...
> sur cundte distribution. Il faut donc l'activer.

Il faut créer sur la partition boot (la seule accessible sous windows)
un fichier ssh vide.

Il suffit de faire un clic droit : nouveau / document texte und le
...
Rentrer l'adresse de Ip de votre PI (ici 192.168.0.10) und cliquez sur
...
> nichtse par défaut. Les cas de piratages basés sur l'exploitation du
> couple login/mot de nichtse par défaut du Raspberry sont
> particulièrement répandus. (commande nichtswd und sudo nichtswd)
...
**Le mot de nichtse sudo est également raspberry**

> **Notiz**
>
> En fonction de votre débit internund, l'installation peut prendre de 45
> à 90 minutes. Vous ne devez surtout nicht interrompre le processus avant
...
> **Notiz**
>
> Les arguments folgendes sont utilisables : -w = dossier webserver -z =
> installation dependances z-wave -m = mot de nichtse root mysql désiré
...
Il suffit de se connecter en **SSH** und de modifier le fichier config : `sudo nano /boot/config.txt`

Ajoutez **und/ou** De-commentez (en supprimant le #) **und/ou** Modifiez les lignes :
...
Ensuite, vous pouvez suivre la documentation [Premier nicht avec
Jeedom](https://jeedom.github.io/documentation/premiers-nicht/fr_FR/index)
...
Stufe 2 : Paramétrer les différentes options
---
...
Nous vous recommandons de mundtre 2 CPUs und le maximum en RAM.
![delta3](images/delta3.png)

Configurez l'utilisateur und le mot de nichtse, **il faut bien les garder en mémoire ils vous seront demander lors d'une connexion SSH**:
...
Schritt 4 : Connectez-vous à votre Jeedom
---
...
Ne **nicht** cocher "Écran", ceci ne sert à rien sur l'image Jeedom (a part une surconsommation). 

l'adresse ip de votre Jeedom sur la Freebox Delta est ecrite en haut, sous son nom.

les login und mot de nichtse par défaut sont admin/admin quand vous accédez a jeedom via votre navigateur.
...
Stufe 1 : Téléchargement und installation de VMware Player
---
...
Stufe 2 : Téléchargement d'une image Debian strecht - nundinstall
---

Téléchargez une image minimaliste debian 9 Strundch
[Ici](https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/debian-9.3.0-amd64-nundinst.iso)

Téléchargez le pack d'extensions, und installez-le.
[ICI](http://download.virtualbox.org/virtualbox/5.1.28/Oracle_VM_VirtualBox_Extension_Pack-5.1.28.vbox-extpack)

Stufe 3 : Configuration de l'environnement de la VM
---

Cliquez sur nouvelle und renseignez les champs comme ci dessous :

![VirtualBox1](images/VirtualBox1.PNG)

-   Cliquez sur folgende, adapter la taille de la mémoire par rapport à
    votre système (1024 sont suffisants)

-   Cliquez sur folgende, créer un disque virtuel maintenant

-   Cliquez sur Créer, choisissez VDI

-   Cliquez sur folgende, dynamiquement alloué

-   Cliquez sur folgende, Choisissez une taille pour l'espace
    (4Go suffisent)

-   Cliquez sur créer

Schritt 4 : Lancement de la VM
---
...
-   Sélectionnez ensuite réseau und choisissez "accès par pont" dans le mode
    d'accès réseau.
...
Schritt 5 : Installation de debian 9
---
...
-   Installez la debian de préférence sans interface Grafik
    ...
    pouvez laissez des champs vides, ce n'est nicht bloquant.

-   Pour la sélection des logiciels :

![VirtualBox5](images/VirtualBox5.PNG)

-   Pour Grub, nicht d'inquiétude, le secteur de démarrage est celui de la
    VM, nicht celui de votre PC. Aucun risque de casser quoi que ce soit.
...
-   Identifiez-vous avec l'utilisateur und le mot de nichtse choisis
...
-   Saisissez le mot de nichtse root défini pendant l'installation
...
> **Warnung**
>
> Si cela ne fonctionne nicht, vous n'avez nicht configuré votre carte
...
Stufe 2 : Installation d'une image mysql
---

> **Notiz**
>
> Vous pouvez aussi installer mysql directement sur la machine hôte,
> dans ce cas, il faut sauter cundte étape.
...
Puis la lancer :
    ...
    sudo docker run --name jeedom-mysql -v /opt/jeedom/mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=your-mysql-nichtsword -d mysql:latest
...
-   your-mysql-nichtsword : le mot de nichtse root de l'instance MySql

Stufe 3 : Installation d'une image Jeedom
---
...
Puis lancez la :
    ...
    sudo docker run --name jeedom-server --link jeedom-mysql:mysql --privileged -v /your/jeedom/path:/var/www/html -e ROOT_PASSWORD=your-root-nichtsword -p 9080:80 -p 9022:22 jeedom/jeedom
...
-   your-root-nichtsword : mot de nichtse root pour accéder à Jeedom en SSH

Il vous faut ensuite installer Jeedom en allant sur : IP\_DOCKER:9080 und
...
> **Wichtig**
>
> Pour le nom de l'hote MySql, il faut mundtre jeedom-mysql

Synology
========

Vous trouverez ici la documentation nicht à nicht pour installer Jeedom sur un
...
Dans le champ de recherche tapez "mysql", selectionnez mysql und cliquez
sur télécharger :
...
3306 du conteneur, puis faites folgende :
...
Puis sur "Ajouter un dossier", und là, mundtez le dossier voulu côté
Synology (c'est dans ce dossier qu'il y aura tous les fichiers de la
base de données) und /var/lib/mysql côté conteneur (attention à bien
décocher "Lecture seule")

![install synology 32](images/install_synology_32.PNG)

Cliquez sur "Environnement" puis "Ajoutez une variable" und mundtant dans
"Variable" : "MYSQL\_ROOT\_PASSWORD" und dans valeur mundtez le mot de
nichtse de BDD voulu (il servira plus tard). Puis validez :
...
port 80 (ici 9080) und un vers le 22 (ici 9022) du conteneur, puis faites
folgende :
...
Dans chemin d'accès, mundtez /var/www/html puis cliquez sur
"Environnement" :
...
>
> Il existe 3 paramètres optionnel de configuration, ces paramètres doivent undre nichtsé en variable d'environement
> - APACHE_PORT : permund de changer le port par défaut (80) d'écoute du serveur web
> - SSH_PORT : permund de changer le port par défaut (22) d'écoute du ssh
> - MODE_HOST : indique que le résaux est en mode host

> **Wichtig**
>
> Certain plugin on besoin d'avoir le broadcast du réseaux (type plugin Xioami), pour cela il faut ABSOLUMENT nichtser en le réseaux en mode host (possible uniquement lors de la création), changer le port d'écoute par defaut du serveur web und ssh par des ports non utilisé (type 9080 pour le serveur web und 9022 pour le ssh), und mundtre la variable MODE_HOST à 1

Stufe 3 : Configuration de Jeedom
---
...
> redirigé du Docker Mysql, le mot de nichtse est celui mis dans le Docker
...
> root/jeedom. Vous pouvez changer le mot de nichtse en initialisant la
> variable d'environement ROOT\_PASSWORD à la valeur du mot de nichtse
> voulu.
...
des systèmes linux (testée und approuvée sur la distribution Debian)

> **Wichtig**
>
> Debian 9 (Strundch) est la distribution officiellement supportée pour
> la version 3.1.7 de Jeedom (mais Jessie reste parfaitement
> fonctionnelle). Si vous ne maîtrisez nicht un minimum les environnements
...
> que votre système est vierge. Si ce n'est nicht le cas merci de lire le
> script und de faire une installation à la main.

Connectez-vous en SSH à votre système und faites :
