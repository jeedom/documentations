...
-   **** : Mit dieser Option können Sie das Bedienfeld anzeigen (Startmenü → Kamera) und alle Ihre Kameras anzeigen (siehe unten)). Vergessen Sie nicht, das Panel in der Konfiguration des Plugins zu aktivieren, um später darauf zugreifen zu können.

## Compatibilité

Sie finden [](https://compatibility.jeedom.com/index.php?v=d&p=home&plugin=camera) die Liste der Module, die mit dem Plugin kompatibel sind
...
-   **** : macht es auf dem Dashboard sichtbar.
-   **** : die lokale IP-Adresse Ihrer Kamera.
-   **** : der Anschluss, an den die Kamera angeschlossen werden soll.
-   **** : das Kommunikationsprotokoll Ihrer Kamera (http oder https).
...
-   **** : ermöglicht Ihnen zu sehen, ob jeedom den Videostream wiederherstellt (denken Sie daran, vor dem Testen zu speichern, um zu sehen, ob Ihre Änderungen funktionieren)
...
-   **Modell** : ermöglicht die Auswahl des Kameramodells.  : Wenn Sie dies ändern, werden Ihre Konfigurationseinstellungen überschrieben.

> ****
>
> Sie können finden [](https://www.ispyconnect.com/cameras) Konfiguration für viele Kameras und daher in der Lage sein, Ihre Kamera selbst in Jeedom zu integrieren, wenn es nicht in der Liste ist
...
-   **** Befehl (Verwendung mit Befehlen vom Typ Info, um beispielsweise Kamerabewegungsinformationen über die API an Jeedom zu senden, siehe unten)
-   **** des Befehls mit der Möglichkeit, stattdessen ein Symbol einzufügen (um es zu löschen, müssen Sie auf das betreffende Symbol doppelklicken)
-   **** und Befehlssubtyp
...
-   **** : Ermöglicht die Anzeige der Reihenfolge oder nicht im Dashboard
...
>
>Die Datensätze werden standardmäßig in Plugins / camera / data / records / gefunden**, Seien Sie vorsichtig, dies kann variieren, wenn Sie Jeedom gebeten haben, sie an anderer Stelle zu speichern.
