...
- Aktion : `json::{"temperature":25}` zum Senden von „{"temperatur":25}` auf dem „set“-Thema des Geräts, um einen Befehl zu einem Unterthema zu senden, müssen Sie nur „arm_mode“ in das Unterthema und „json“ zum Beispiel einfügen::{"mode:"arming_stay"}` in der logischen ID zum Senden von `{"mode:"arming_stay"}` auf `set/arm_mode`
