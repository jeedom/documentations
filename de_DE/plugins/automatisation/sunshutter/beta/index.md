...
- **** :
    - **** : Wird verwendet, um die Ausrüstung zu aktivieren.
    - **** : Ausrüstung sichtbar machen.
...
>* : das System schließt den Rollladen, dann öffnen Sie ihn einige Minuten später manuell, dann wird die automatische Verwaltung nur durchgeführt, wenn **Gewinnen Sie die Kontrolle zurück** ist um "****" oder dass die Erholungszeit abgelaufen ist oder dass die Bestellung **Geschäftsführung übernehmen** wird ausgelöst.*
...
>****
>
>Die hier erwähnten "Modi" beziehen sich auf die auf der Registerkarte definierten Modi **** von jedem Plugin-Equipment **Rollladenmanagement** und beziehen sich in keiner Weise auf Plugin-Modi ****.
...
- **** : Der Breitengrad des Gebäudes oder Fensterladens.
- **** : Der Längengrad des Gebäudes oder Fensterladens.
- **** : Die Höhe des Gebäudes oder Fensterladens.
...
- **** : Die Bedingung, die erfüllt sein muss, damit die Regel gültig ist *(facultatif)*.
- **** : Bei Eingabe ist die Bedingung nur gültig, wenn sich der Rollladen derzeit im angegebenen Modus befindet. Es ist möglich, mehrere Modi anzugeben, indem Sie sie durch Kommas trennen *(facultatif)*.
...
- **** : Aktivieren Sie das Kontrollkästchen, um die automatische Verwaltung auszusetzen, solange die Regel gültig ist.
- **** : Die gewünschte Position in Prozent, wenn die Regel gültig ist *(leer = keine Aktion)*.
- **** : Label, das mit der Validierung der Ausnahmeregel verknüpft ist *(facultatif)*.
...
- **** : Zu der Position komplementäre Bedingung, die erfüllt sein muss, damit die Positionierung des Rollladens validiert wird *(facultatif)*.
- **** : Geben Sie die Azimutgrenzen der Sonne in Grad ein, zwischen denen die Bedingung gültig ist.
- **Höhe** : Geben Sie die Sonnenhöhengrenzen in Grad ein, zwischen denen die Bedingung gültig ist.
- **** : Die gewünschte Position in Prozent, wenn die Positionierungsbedingungen (und ggf. zusätzliche) gültig sind.
- **** : Label verbunden mit der Validierung der Positionierungsbedingung *(facultatif)*.

![Conditions](../images/sunshutter_conditions.png)

>****
>
>Der Standort [](https://www.){:target = "\_ blank"} ermöglicht, nach Eingabe Ihrer Adresse, die Position der Sonne und die Azimut- und Elevationswinkel entsprechend den Tagesstunden zu visualisieren *(zieh einfach die kleine sonne hoch)*.

# Programmation

Diese Registerkarte erscheint nur, wenn das Plugin **** ist auf Ihrem Jeedom installiert. Es listet die Programmierung des Plugins auf **** auf die automatische Verwaltung dieser Komponente einwirken, um beispielsweise eine Unterbrechung und eine manuelle Wiederaufnahme während der Mittagsschlafzeiten eines Kindes zu planen.
...
- **** : Aktualisierung von Bestellungen erzwingen **Sonnenazimut**  **Sonnenaufgang**.
>Es ist natürlich möglich, die Werte der Befehle zu verwenden **Azimut Sonne**  **Sonnenaufgang** anderswo in Jeedom.
...
- **** : Label, das einer Ausnahme oder einer gültigen Positionierungsbedingung entspricht.
- **** : Unterbricht die automatische Rollladenverwaltung.
- **Aussetzen (Label)** : Aktueller Stand der automatischen Rollladenmanagement Aussetzung *(Keine / Manuell / Auto)*.
- **** : Neustart der automatischen Verwaltung erzwingen.
>Dies ist der Befehl, der ausgeführt werden muss, um zur automatischen Steuerung zurückzukehren, wenn Sie die Position Ihres Rollladens manuell geändert und konfiguriert haben **Gewinnen Sie die Kontrolle zurück** beim "****".

- **** : Aktueller Verschlussmodus.
...
>****
>
>Sie können beispielsweise einen Modus definieren **** und ein Modus **** und passen Sie das Öffnen und Schließen Ihres Verschlusses gemäß diesen 2 Modi an.

# Santé

Das Plugin **Verwaltung von Rollläden** hat ein fenster ****, auf der Seite „Allgemeine Ausrüstung“, auf der Sie die Konfigurationen der einzelnen automatischen Verwaltungen auf einen Blick sehen können.
