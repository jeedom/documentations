...
Il faut aller sur le panneau de Konfiguration puis "Services de
...
(j'ai pas trouvé l'info sur le client web) et aller dans Konfiguration →
stockage :
...
ensuite retourner avec le client lourd dans Konfiguration → stockage :
...
vous faudra sûrement vous connecter avec votre compte VMware). Auf dem
...
Ensuite sur le NAS dans le panneau de Konfiguration puis portail
...
> hinzufügen. Ensuite n'oubliez pas de cliquer sur configurer pour
