...
Auch hier sind Sie nicht verpflichtet, diesen Mechanismus zu verwenden, es steht Ihnen frei, etwas anderes zu erstellen (z. B. Server http), aber es ist das, was von Jeedom grundlegend bereitgestellt wird, es ist leicht und es funktioniert sehr gut.
...
Anstelle von `print 'read'` ist es also an Ihnen, die relevanten Elemente der Nachricht zu lesen, die Ihr Plugin gesendet haben wird, und die Aktionen auszulösen oder Ihre Klassen oder Methoden aufzurufen, die für Ihr Plugin spezifisch sind.
...
Es ist sehr gut, einen Daemon zu haben und seine Struktur zu verstehen, aber einige Schlüsselelemente fehlen, damit Ihr Plugin (PHP-Code) diesen Daemon steuern kann und damit der Kern auch darüber informiert wird, dass er existiert.
...
Wir werden später die Verwendung von `hasDependency` und `maxDependencyInstallTime` sehen.
