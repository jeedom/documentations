...
Im [Lernprogramm](/de_DE/dev/tutorial_plugin) und der [Dokumentation](/de_DE/dev/plugin_template) Sie haben gelernt, wie Sie Ihr erstes Plugin mit relativ einfachen Aktionen programmieren, die vom Benutzer über einen Aktionsbefehl oder durch eine vom Kern geplante Aufgabe (die crons).
...
In der Plugin-Vorlage finden Sie die Grundlagen zum Implementieren eines Daemons in Python, und dies ist das Beispiel, das wir in dieser Dokumentation verwenden werden. Es steht Ihnen jedoch frei, Ihren Daemon in der Sprache Ihrer Wahl zu entwickeln, vorausgesetzt, dass er ausgeführt werden kann [Plattformen, die von Jeedom unterstützt werden](/de_DE/compatibility/).
...
Im Vorlagen-Plug-in hieß das Daemon-Verzeichnis „demond“, und der Daemon selbst heißt „demond.py“.
Diese Namen sind willkürlich, Sie können sie frei ändern.
Die Konvention besteht darin, die Plugin-ID gefolgt vom Buchstaben „d“ zu verwenden. Was zum Beispiel für das Plugin `blea` das Verzeichnis `./resources/blead/` die unter anderem die Datei `blead.py“, wobei diese Datei der Ausgangspunkt des Daemons ist.
...
Wenn die Warteschlange nicht leer ist, laden wir den json und prüfen, ob der mit der Nachricht erhaltene API-Schlüssel mit dem übereinstimmt, der beim Starten des Daemons (`_apikey`) empfangen wurde. Dann können wir die Nachricht lesen und unsere Aktionen in try/ ausführen. außer:
...
```php
    öffentliche statische Funktion daemon_start() {
        self::Dämon_Stop();
...
Die Zeilen vom Typ `echo 60 > ${PROGRESS_FILE}` werden offensichtlich verwendet, um den Fortschritt zurückzugeben: Um den Benutzer zu beruhigen, setzen wir es regelmäßig ein, bis wir 100 erreichen (normalerweise stressen sie sich, wenn es 100 überschreitet, also vermeiden wir es).
...
```php
    öffentliche statische Funktiondependency_info() {
        ...
        if (file_exists(jeedom::getTmpOrdner(__CLASS__) . '/dependency')) {
            ...
            if (exec(system::getCmdSudo() . system::get('cmd_check') . '-Ec "python3\-Anfragen|python3\-sinnlich|python3\-bs4"') < 3) { // Paketliste und Gesamtzahl anpassen
                $return['state'] = 'nok';
...
In diesem Beispiel testen wir das Vorhandensein von apt-Paketen: `system::getCmdSudo() . system::get('cmd_check') . '-Ec "python3\-Anfragen|python3\-sinnlich|python3\-bs4"'`. Hier wollen wir `python3-requests`, `python3-voluptuous` und `python3-bs4` und daher muss der Befehl 3 zurückgeben, daher der Vergleich: `< 3`.
