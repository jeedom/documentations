...
Die `JEEDOM_SOCKET_MESSAGE`-Variable ist eine Python-`Warteschlange()`, die von der `jeedom_socket()`-Klasse betrieben wird, wie zuvor gesehen.
...
Dies ist die Funktion, die ich selbst benutze (@Mips) in jedem meiner Plugins mit einem Daemon, ich füge es hier ein und du machst damit, was du willst;-)
