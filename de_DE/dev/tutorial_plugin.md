...
Ich öffne die D.esktop /. PHP /. D.esktop-Datei.PHP zu beheben.
...
Derzeit werden weder auf der Befehlsregisterkarte s noch im Widgund Befehle angezeigt.
...
Jetzt muss noch die Funktion randomVdm () gestartund werden . D.azu rufen wir die eqLogic (Ausrüstung) aus dem Startbefehl ab und starten die Funktion
...
`` ''
    öffentliche Funktion postUpdate () {
		self::CronHourly ($ this-> getIdentifikation ());; /./. Startund die CronHourly-Funktion mit der eqLogic-ID
    }
`` '' 
...
Das Widgund ist keine leichte Aufgabe, aber wir bleiben vorerst beim Standard-Widget.

Wenn Sie nichts berührt haben, die Ausrüstung aktiviert und sichtbar ist, nimmt das Widgund die gesamte Bildschirmbreite ein. Also werden wir es ändern.
...
Aber ich habe meine Brille nicht und momentan kann ich sie beim Rendern im Widgund nicht lesen ...
...
Dafür öffne ich die D.atei vdm.class.php , fonction postSave() und j'ajoute le template « tile » pour la commande « story » en appliquant la méthode setTemplate()
...
Es ist besser, aber das Widgund nimmt immer die Breite des D.ashboards an. Also werden wir korrigieren. Von défaut la largeur du widgund de l'équipement(eqLogic) und la hauteur sont à « auto ».
