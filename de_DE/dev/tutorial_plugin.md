...
-Créer un équipement « vdm1 » In cliquant sur le +. AktivierIn Sie es und machIn Sie es sichtbar. WählIn Sie ein Objekt aus und die Ausrüstung muss im D.ashboard angezeigt werdIn (je nach Objekt)..
...
-Créer un autre équipement « vdm2 » In cliquant sur le +. Auf der Registerkarte Befehle wurdIn die Befehle angezeigt. AktivierIn Sie es und machIn Sie es sichtbar. WählIn Sie ein übergeordnetes Objekt aus und sehIn Sie, wie es im D.ashboard aussieht.
...
C'est hier qu'on va définir ce qu'il va se passer quand on lance la commande « Rafraîchir ». D.ie Klasse vdmCmd hat alle MethodIn der Klasse cmd (Core jeedom) geerbt.
...
Bevor wir undwas unternehmen, überprüfIn wir, ob das Gerät aktiv ist
...
Anschließend könnIn Sie die Häufigkeit des Cron Intsprechend der Wichtigkeit der Wiederherstellung Ihrer InformationIn ändern.
...
Ich werde mir die Zeit nehmen, um hinzuzufügen, wie ein benutzerdefiniertes Cron Intsprechend der Ausrüstung eingerichtund wird .
...
Für einIn Vonameter / eine Option innerhalb des Plugins verwendIn wir eine der MethodIn der Klasse eqLogic des Kerns : setConfiguration () durch HinzufügIn der gewünschtIn Vonameter. Zum Beispiel :

````
$this->setConfiguration("type","mon_type"); // si on veut  définir un Vonamètre type de valeur mon_type. Comme on a défini la largeur du widgund via la méthode setDisplay()
````
...
````
<?php include_file('core', 'plugin.template', 'js');?>//Chargement du fichier core/js/plugin.template.js (In Vontant de la racine du site)
````
...
# Verwendung von OptionIn / Vonametern
...
Jetzt werdIn wir unserIn Vonameter in der Funktion randomVdm () in der D.atei vdm.class.php berücksichtigen
