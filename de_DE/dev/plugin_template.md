...
Letzter wichtiger Punkt auf der Konfigurationsseite: Dieser kann so viele Geräte und Steuerungen enthalten wie nötig. Allerdings müssen einige Regeln beachtet werden :
...
- ``jeedom.cmd.changeType(\$('\#table\_cmd tbody tr:last'), init(\_cmd.subType))`` Dadurch können Sie den Subtyp mit dem richtigen Wert initialisieren

Andere Javascript-Funktionen können verwendet werden :

- ``printEqLogic`` die als Parameter das gesamte Objekt der Ausrüstung nimmt (nützlich im Fall der Datenverarbeitung, bevor sie zurückgegeben werden). Es wird aufgerufen, wenn Gerätedaten angezeigt werden
- ``saveEqLogic`` welches als Parameter das Ausrüstungsobjekt verwendet, das in der Datenbank gespeichert wird (nützlich, wenn Sie vor dem Speichern eine Verarbeitung durchführen müssen). Und zu guter Letzt: Hier erfahren Sie, wie Sie die JS-Dateien sauber in Ihre PHP-Seite einbinden :
...
Im Modalordner können Sie Ihre PHP-Dateien speichern, die zur Anzeige von Modalen bestimmt sind. So rufen Sie sie von Ihrer Hauptseite aus auf (dieser Code wird in eine JavaScript-Datei eingefügt)) :
...
Mit einem Klick auf den Assistenten-Button rufen Sie die Eingaben ab und rufen anschließend den Assistenten auf. Nach Abschluss der Konfiguration im Assistenten wird das Ergebnis abgerufen und anschließend in den zuvor ausgewählten Eingang geschrieben

# Plugin-Erstellung Teil 4 : Kernordner

Der mit Abstand wichtigste Ordner in Ihrem Plugin. Er kann 4 Unterordner haben.

Notiz : In diesem Teil wird auf Ihre Plugin-ID verwiesen durch : plugin_id

## PHP

Enthält die angehängten PHP-Dateien. Ich habe mir angewöhnt, beispielsweise eine Einschlussdatei einzufügen, wenn Sie natürlich mehrere Klassendateien oder Dateien von Drittanbietern einschließen möchten

## Template

Dieser kann zwei Unterordner enthalten: Dashboard und Mobile. Dies ist ein Ordner, den Jeedom automatisch nach Widgets durchsucht. Wenn Sie also bestimmte Widgets verwenden, sollten Sie deren HTML-Datei hier ablegen

## i18n

Hier sollte Ihre Übersetzung in Form einer JSON-Datei vorliegen (am besten schauen Sie sich zum Beispiel das Plugin an [zwave](https:github.comjeedomplugin-openzwave) um das Dateiformat anzuzeigen)
...
versuchen {
    ...
    include_file('core', 'authentication', 'php');
     ...
     (!')) {
        neue Ausnahme werfen (__('401 - Unauthorized Access', __FILE__));
    }
    ...
    throw new Exception(__('Keine passende Methode : ', __FILE__) . init('action'));
    *     * *********Ausnahme abfangen*************** *
} Fang (Ausnahme $e) {
...
Speichern des Widgets im Cache: damit wir es bei der nächsten Anfrage schneller bereitstellen können, können wir hier die 0 beachten, die eine unendliche Lebensdauer anzeigt, ansonsten wird die Dauer in Sekunden angegeben (wir werden im nächsten Teil sehen, wie das Wetter-Plugin sein Widget aktualisiert).
