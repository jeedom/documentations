...
Die Liste der Ausrüstungs- und Befehlseigenschaften finden Sie hier (um zu sehen, welche Eigenschaften JSON sind, schauen Sie sich einfach den Getter oder Setter an; wenn er zwei Parameter benötigt, dann ist er JSON))

Ein letzter wichtiger Punkt auf der Konfigurationsseite: Hier können so viele Geräte und Bedienelemente enthalten sein, wie nötig. Es gibt jedoch einige Regeln zu beachten :

Alle Elemente mit der Klasse `eqLogicAttr` müssen sich innerhalb eines Elements mit der CSS-Klasse `eqLogic` befinden. Dasselbe gilt für Elemente mit der CSS-Klasse `cmdAttr`, die sich innerhalb eines Elements mit der Klasse `cmd` befinden müssen. Alle Befehle für ein Gerät müssen sich in dem Element mit der entsprechenden eqLogic-Klasse befinden

## JS

Alle JS-Dateien müssen sich im JS-Ordner befinden (ganz einfach) !!!). Es wird empfohlen, es mit der gleichen ID wie Ihr Plugin zu benennen (im Konfigurationsbereich; für das Panel können Sie es nach Belieben benennen)). Diese JS-Datei (die Plugin-Konfigurationsdatei) muss mindestens eine addCmdToTable-Methode enthalten, die das hinzuzufügende Befehlsobjekt als Parameter entgegennimmt. Hier ist ein einfaches Beispiel :
...
- ``jeedom.cmd.availableType()`` wird eine Auswahlliste mit den bekannten Datentypen (derzeit Aktion und Information) einfügen)
- ``<span class="subType" subType="' + init(\_cmd.subType) + '"><\span>`` : die Position, an der die Subtypauswahl platziert werden soll
- ``jeedom.cmd.changeType(\$('\#table\_cmd tbody tr:last'), init(\_cmd.subType))`` wodurch der Subtyp mit dem korrekten Wert initialisiert werden kann

Es können auch andere JavaScript-Funktionen verwendet werden :

- ``printEqLogic`` wobei das gesamte Gerät als Parameter verwendet wird (nützlich bei der Datenverarbeitung vor der Ausgabe)). Es wird aufgerufen, wenn Gerätedaten angezeigt werden
- ``saveEqLogic`` Diese Funktion benötigt als Parameter das Geräteobjekt, das in der Datenbank gespeichert werden soll (nützlich, wenn vor dem Speichern eine Verarbeitung erforderlich ist). Abschließend noch ein Hinweis zur sauberen Einbindung von JS-Dateien in Ihre PHP-Seite :
...
Das erste Argument gibt den Ordner an, in dem sich die Datei befindet (beachten Sie, dass dies der übergeordnete Ordner des CSS-Ordners ist), das zweite den Namen Ihrer CSS-Datei, das dritte teilt Jeedom mit, dass es sich um eine CSS-Datei handelt, und das letzte, in welchem Plugin sie sich befindet.

## MODAL

Im Ordner „modal“ können Sie Ihre PHP-Dateien speichern, die zur Anzeige von Modalfenstern bestimmt sind. So rufen Sie sie von Ihrer Hauptseite aus auf (dieser Code gehört in eine JavaScript-Datei)) :
...
Wenn Sie auf die Schaltfläche „Assistent“ klicken, rufen Sie die einzugebenden Daten ab und rufen dann den Assistenten auf. Sobald die Konfiguration im Assistenten abgeschlossen ist, wird das Ergebnis abgerufen und anschließend in das zuvor ausgewählte Eingabefeld geschrieben

# Plugin-Erstellung Teil 4 : Core-Ordner

Der mit Abstand wichtigste Ordner in Ihrem Plugin; er kann 4 Unterordner enthalten.

Notiz : In diesem Abschnitt wird die ID Ihres Plugins wie folgt referenziert: : Plugin-ID

## PHP

Enthält die ergänzenden PHP-Dateien; ich habe mir angewöhnt, beispielsweise eine Include-Datei einzufügen, wenn man mehrere Klassendateien oder Dateien von Drittanbietern einbinden muss

## Template

Dieser Ordner, der die Unterordner „Dashboard“ und „Mobil“ enthalten kann, wird von Jeedom automatisch nach Widgets durchsucht. Wenn Sie bestimmte Widgets verwenden, müssen Sie daher deren HTML-Dateien hier ablegen

## i18n

Hier sollte Ihre Übersetzung als JSON-Datei gespeichert werden (am besten suchen Sie beispielsweise im Plugin-Verzeichnis) [Z-Wave](https:github.comjeedomplugin-openzwave) um das Dateiformat anzuzeigen)

## ajax

Dieser Ordner ist für all Ihre AJAX-Dateien; hier ist eine AJAX-Dateivorlage :
...
versuchen {
    ...
    include_file('core', 'authentication', 'php');
     ...
     (!')) {
        throw new Exception(__('401 - Nicht autorisierter Zugriff', __FILE__));
    }
    ...
    throw new Exception(__('Keine passende Methode gefunden') : ', __FILE__) . init('action'));
    *     * *********Ausnahme abfangen*************** *
} catch (Exception $e) {
...
Zur Definition der Jeedom-Klassen empfehle ich Ihnen, dies zu konsultieren [Website](https:doc.jeedom.comdevphpdoc4.0)

Die einzige obligatorische Methode ist die Instanzmethode der Klasse `cmd execute`; hier ist ein Beispiel mit dem SARAH-Plugin :

````php
public function execute($_options = array()) {
       (!isset($_options['title']) && !isset($_options['message'])) {
          throw new Exception(__("Der Titel oder die Nachricht dürfen nicht beide leer sein", __FILE__));
      }
...
Hier gibt es einige interessante Dinge :

Um die angeforderte Version in eine Dashboard- oder mobile Version umzuwandeln (z. B. wird aus mview eine mobile Version), können beispielsweise Objektnamen zu den Ansichten hinzugefügt werden)
...
Hier werden die zuvor ausgefüllten Tags in der `$replace`-Anweisung des HTML-Codes durch die Werte ersetzt
...
Dies ermöglicht es Ihnen, den Wert im Tag einzufügen, jedoch nur, wenn der Befehl erfolgreich abgerufen wurde
...
Das Widget wird im Cache gespeichert: Um sicherzustellen, dass die Daten bei der nächsten Anfrage schneller bereitgestellt werden, beachten Sie die 0 hier, die eine unendliche Lebensdauer anzeigt; ansonsten wird die Dauer in Sekunden angegeben (wir werden im nächsten Abschnitt sehen, wie das Wetter-Plugin sein Widget aktualisiert)).
...
Sie müssen Jeedom außerdem mitteilen, welche Anpassungsoptionen Ihr Widget bietet. Es ist etwas komplex (wenn auch nicht völlig), aber normalerweise flexibel und einfach umzusetzen.

Es funktioniert auf Ihrem Gerät oder Steuerelement genauso; es handelt sich um ein statisches Attribut der Klasse `$_widgetPossibility`, die ein mehrdimensionales Array sein muss. Kompliziert wird es jedoch, wenn eine Dimension des Arrays den Wert „true“ oder „false“ hat. Er geht dann davon aus, dass alle möglichen Kinder diesen Wert haben (ich werde ein Beispiel geben)).

Zunächst die Fälle, in denen Sie es verwenden sollten: Wenn Ihre Klasse, die von eqLogic oder cmd erbt, über eine `toHtml`-Funktion verfügt, brauchen Sie nicht weiterzulesen.

### Vorher- und Nachher-Methode

Beim Erstellen oder Löschen von Objekten (Ausrüstung, Steuerung oder Sonstiges) in Jeedom können vor/nach der Aktion verschiedene Methoden aufgerufen werden :

- ``preInsert`` ⇒ Methode, die vor der Erstellung Ihres Objekts aufgerufen wurde
- ``postInsert`` ⇒ Diese Methode wird aufgerufen, nachdem Ihr Objekt erstellt wurde
- ``preUpdate`` ⇒ Diese Methode wird aufgerufen, bevor Ihr Objekt aktualisiert wird
- ``postUpdate`` ⇒ Diese Methode wird aufgerufen, nachdem Ihr Objekt aktualisiert wurde
...
- ``preRemove`` ⇒ Methode, die vor dem Löschen Ihres Objekts aufgerufen wurde
- ``postRemove`` 
